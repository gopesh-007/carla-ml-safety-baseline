...
\begin{itemize}
  \item \textbf{Traffic-light semantics (H-2):} presence alone gives no red/green state. Add a
  validated state classifier or infrastructure signal; until then, unknown state must default to stop.
  \item \textbf{Single-camera dependence (H-1,H-3,H-4):} one occlusion or camera fault removes all
  perception. Add independent radar/LiDAR or another safety-rated sensing channel and camera diagnostics.
  \item \textbf{Classification without localization (H-1,H-5):} frame-level presence is not a
  complete driving perception interface. Object localisation, tracking and path relevance are required.
  \item \textbf{Offline rather than closed-loop evidence (all hazards):} the experiments measure
  classifiers on saved frames. They do not establish 10\,Hz latency, stopping distance, controller
  stability, operator response or collision rate. Closed-loop scenario tests are necessary.
  \item \textbf{Simulator-to-reality gap (H-3,H-4):} no evidence supports transfer from the supplied
  simulated images to real public roads.
\end{itemize}
...
\textbf{Frames are not the same as independent driving scenarios.} The standard test split contains
3,600 labelled frames, but neighbouring CARLA frames may come from the same driving sequence and
share scenery, weather and road users. I therefore report the evaluated frame count without treating
it as 3,600 statistically independent safety scenarios. A scenario-level evaluation would require
separate runs, defined initial conditions and outcomes such as collision or successful stopping.

\textbf{The adversarial result is limited to the implemented attack.} V-2 used untargeted one-step
FGSM on a deterministic sample of 100 validation images per model with seed 42. The observed recall
drops are sufficient to reject the stated V-2 bound for that protocol. They do not estimate the
real-world frequency of adversarial inputs or establish behaviour under PGD, adversarial patches,
physical attacks, compression, camera artefacts or other perturbation models.

\textbf{The calibration result is conditional on in-distribution data.} Temperature was selected on
the validation split and ten-bin ECE was then computed on the standard test split. This separation
avoids fitting the temperature on the reported test result, but the resulting V-3 pass applies only
to the tested dry/day/source-town distribution. For pedestrian, validation NLL was still decreasing
at temperature 3.0, the edge of the search grid, so the reported value is the best tested setting
rather than a demonstrated unconstrained optimum. ECE also depends on the binning choice, and no
calibration claim was verified for fog, night, Town-01 or later dataset drift.

\textbf{OOD and ODD evidence cover only selected factors.} The feature detector used $k=5$, 1,000
validation images as the reference set, an independent 1,000-image dry/day test subset as ID, and
1,000 images from each OOD condition. AUROC measures ranking across each complete comparison; no
runtime operating threshold or fallback response was validated. The ODD coverage
calculation used weather, lighting and town because those factors were available in the supplied
metadata. It did not measure coverage over object distance, occlusion, object size, traffic density,
glare, blur, camera contamination or combinations of these conditions. The reported 33.3\%
three-way coverage is therefore a description of the selected factors, not of the complete driving
environment.

\textbf{The thresholds are project verification criteria.} The recall, recall-drop, ECE and AUROC
limits define the checks in this safety case. They are not presented as legal certification limits
or universal acceptance criteria for autonomous-driving systems. A failed threshold is sufficient
to leave the linked safety constraint open, while a passed threshold supports only the particular
metric, dataset and experimental protocol that were evaluated.
